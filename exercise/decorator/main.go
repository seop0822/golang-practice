...
import (
	"exercise/decorator/ciper"
	"exercise/decorator/lzw"
	"fmt"
)
...
func (self *EncryptComponent) Operator(data string) {
	encryptData, err := ciper.Encrypt([]byte(data), self.key)
	if err != nil {
		panic(err)
	}
...
func (self *DecryptComponent) Operator(data string) {
	decryptData, err := ciper.Decrypt([]byte(data), self.key)
	if err != nil {
		panic(err)
	}
